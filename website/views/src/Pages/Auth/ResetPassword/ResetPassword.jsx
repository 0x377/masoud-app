...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.type === 'auth/resetPassword/fulfilled') {
        ...
        const errorMessage = result.payload || 'حدث خطأ أثناء إعادة تعيين كلمة المرور';
        
        if (errorMessage.includes('غير صالح') || errorMessage.includes('انتهت')) {
          setTokenValid(false);
  ...
  const handleResendResetLink = async () => {
    try {
      ...
      const resetRequest = JSON.parse(localStorage.getItem('reset_request') || '{}');
      const email = resetRequest.contact || '';
      
      if (!email) {
        setErrors({ 
          general: 'لم يتم العثور على البريد الإلكتروني. يرجى طلب رابط جديد من صفحة نسيت كلمة المرور.' 
        });
        return;
      }
