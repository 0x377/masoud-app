...
//   // Create database triggers for security
//   await queryInterface.execute(`
...
//         id, user_id, severity, action, description, 
//         affected_user_id, affected_table, affected_record_id, created_at
//       ) VALUES (
//         UUID(),
//         OLD.id,
//         'HIGH',
//         'USER_DELETION_ATTEMPT',
//         CONCAT('Attempt to delete user: ', OLD.email),
//         OLD.id,
//         'users',
//         OLD.id,
//         NOW()
...
//       SET MESSAGE_TEXT = 'Direct deletion not allowed. Use soft delete by updating deleted_at field.';
//     END;
//   `);
...
//             WHEN NEW.status IN ('DEACTIVATED') THEN 'MEDIUM'
//             ELSE 'LOW'
//           END,
//           CONCAT('STATUS_CHANGE_', NEW.status),
//           CONCAT('User status changed from ', OLD.status, ' to ', NEW.status),
//           JSON_OBJECT('old_status', OLD.status),
//           JSON_OBJECT('new_status', NEW.status),
//           NEW.id,
//           'users',
//           NEW.id,
//           NOW()
...
//   // Drop tables in reverse order (due to foreign key constraints)
//   await queryInterface.execute(
//     "DROP TRIGGER IF EXISTS after_user_status_change",
//   );
//   await queryInterface.execute("DROP TRIGGER IF EXISTS before_user_delete");
//   await queryInterface.execute("DROP TABLE IF EXISTS user_permissions");
//   await queryInterface.execute("DROP TABLE IF EXISTS password_history");
//   await queryInterface.execute("DROP TABLE IF EXISTS security_logs");
//   await queryInterface.execute("DROP TABLE IF EXISTS login_histories");
