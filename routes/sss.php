...
// Family Archive Routes (أرشيف العائلة - الخانة الرابعة)
Route::middleware(['auth:sanctum', 'check.user.type:FAMILY_MEMBER,SUPER_ADMIN'])->prefix('archive')->group(function () {
    Route::get('/family-tree', [FamilyController::class, 'getCompleteFamilyTree']);
    Route::get('/meetings', [FamilyController::class, 'getFamilyMeetings']);
    Route::get('/sports-archive', [FamilyController::class, 'getSportsArchive']);
    Route::get('/documents', [FamilyController::class, 'getFamilyDocuments']);
    Route::get('/{cardId}', [CreditCardController::class, 'show']);
    Route::put('/{cardId}', [CreditCardController::class, 'update']);
    Route::delete('/{cardId}', [CreditCardController::class, 'destroy']);
    Route::put('/{cardId}/default', [CreditCardController::class, 'setDefault']);
    Route::post('/{cardId}/transactions', [CreditCardController::class, 'addTransaction']);
    Route::get('/{cardId}/transactions', [CreditCardController::class, 'getTransactions']);
    Route::get('/{cardId}/statement', [CreditCardController::class, 'generateStatement']);
});

// Hall Booking System (حجز القاعة)
...
// Reports System
Route::prefix('reports')->middleware(['check.role:SUPER_ADMIN,EXECUTIVE'])->group(function () {
...
Route::prefix('admin')->middleware(['check.role:SUPER_ADMIN'])->group(function () {
    // User Management
